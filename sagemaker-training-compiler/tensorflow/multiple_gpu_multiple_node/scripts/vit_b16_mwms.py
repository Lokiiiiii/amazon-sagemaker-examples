            ...
            with Image.open(fpath) as img:
                if img.size[0]<INPUT_SHAPE[0] or img.size[1]<INPUT_SHAPE[1]:
                    ...
                    for dim in [0,1]:
                        ...
                        margin[dim] = diff//2
                    array = array[margin[0]:margin[0]+INPUT_SHAPE[0], margin[1]:margin[1]+INPUT_SHAPE[1]]
                    try:
...
if __name__== '__main__':
    parser = argparse.ArgumentParser(description='Configure the VIT Training through Hyperparameters')
    parser.add_argument('--NUM_CLASSES', type=int, default=257,
                        help='Number of classfication categories')
    parser.add_argument('--INPUT_SHAPE', type=int, nargs='+', default = [224, 224, 3],
                        help='Shape of input to VIT')
    parser.add_argument('--BATCH_SIZE', type=int,
                        help='Batch Size to use with the Hardware')
    parser.add_argument('--LEARNING_RATE', type=float, default=0.001,
                        help='Learning rate to use for the Optimizer')
    parser.add_argument('--WEIGHT_DECAY', type=float, default=0.0001,
                        help='Weight decay to use for the Optimizer')
    parser.add_argument('--EPOCHS', type=int, default=1,
                        help='Number of times to loop over the data')
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices(
      (x_train, y_train)).shuffle(30000).batch(args.BATCH_SIZE)
    ...
    model.fit(train_dataset, epochs=args.EPOCHS, batch_size=args.BATCH_SIZE, verbose=2)
